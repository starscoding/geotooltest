...
//			return 307;// 环间
	...
	public static boolean getDistrict(String points, Double lontitude,
			Double latitude) {
		boolean flag = false;
		List<GisHelper.Point> pointList = GisHelper.parsePoints(points);
		if (pointList != null) {
			boolean b = GisHelper
	 ...
	 * @return
	 */
	/*public static List<GisEachMarker> parseGeziData(int xWidth, int yWidth) {

		String[] bounds = Bounds.split(",");
		double lb_x = Double.parseDouble(bounds[0]);
		double lb_y = Double.parseDouble(bounds[1]);
		double rt_x = Double.parseDouble(bounds[2]);
		double rt_y = Double.parseDouble(bounds[3]);
		double x_width = rt_x - lb_x; // 经度长度
		double y_width = rt_y - lb_y; // 纬度长度

		double x_each_width = (X_EACH_WIDTH_1000 * xWidth) / 1000.0;
		double y_each_width = (Y_EACH_WIDTH_1000 * yWidth) / 1000.0;

		int x_num = Integer.parseInt(String.valueOf(
				Math.floor(x_width / x_each_width)).split("\\.")[0]);
		int y_num = Integer.parseInt(String.valueOf(
				Math.floor(y_width / y_each_width)).split("\\.")[0]);

		System.out.println(x_each_width + "-" + y_each_width);

		List<GisEachMarker> rslt = new ArrayList<GisEachMarker>();
		for (int i = 0; i < x_num; i++) {
			for (int j = 0; j < y_num; j++) {
				double longitude_min = lb_x + x_each_width * i;
				double latitude_min = lb_y + y_each_width * j;
				double longitude_max = longitude_min + x_each_width;
				double latitude_max = latitude_min + y_each_width;
				double longitude = Double.parseDouble(LNG_DECF
						.format((longitude_min + longitude_max) / 2));
				double latitude = Double.parseDouble(LAT_DECF
						.format((latitude_min + latitude_max) / 2));
				Integer gisNum = 0;

				String district = "";

				GisEachMarker gisEachMarker = new GisEachMarker(String
						.valueOf(longitude)
						+ "_" + String.valueOf(latitude), district, longitude,
						latitude, longitude_min, latitude_min, longitude_max,
						latitude_max, gisNum);

				// System.out.println(longitude_min+","+latitude_min+","+longitude_max+","+latitude_max+";"+longitude+"-"+latitude);
				rslt.add(gisEachMarker);
			}
		}
	 ...
	 * @return
	 */
/*	public static List<AntennaInfo> parseGzStations(Double longitude,
			Double latitude, List<AntennaInfo> antenna) {

		if (antenna == null || antenna.isEmpty())
			return null;
		List<AntennaInfo> fList = new ArrayList<AntennaInfo>();
		// 一、过滤投诉点周边5公里范围内基站小区
		for (AntennaInfo a : antenna) {
			double distatce = getDistatce(longitude, latitude,
					a.getLongitude(), a.getLatitude());
			if (distatce <= 5000) {
				a.setDistatce(distatce);
				fList.add(a);
			}
		}
		if (fList == null || fList.isEmpty())
			return null;
		List<AntennaInfo> aList = new ArrayList<AntennaInfo>();
		// 二、获取投诉点与正北方向的夹角，采用数学向量知识计算夹角
		for (AntennaInfo a : fList) {
			// 1、定义正北方向向量为（x1，y1），投诉点与基站的方向向量为：（x2,y2）。
			// 其中x1=0，y1=1; x2=TS_Longitude-Longitude，y2=ts_Latitude-Latitude;
			double x1 = 0;
			double y1 = 1;
			double x2 = longitude - a.getLongitude();
			double y2 = latitude - a.getLatitude();
			// 2、根据夹角公式计算投诉点与正北方向的夹角A，公式为：
			// cos（A）=(x1*x2+y1*y2)/[sqr(x12+y12)* sqr(x22+y22)] =a，A=arccos（a）；
			double A = Math.acos((x1 * x2 + y1 * y2)
					/ (Math.sqrt(x1 * x1 + y1 * y1) * Math.sqrt(x2 * x2 + y2
							* y2)));
			// 3、与正北方向角度（angel）修正，由于夹角公式算出来的角度在0-180之间，根据投诉点与基站的方向向量（x2,y2）所处的象限分成两种情况：
			// 1）落在一、四象限（x2>=0）angel=A，2）落在二、三象限（x2<0）angel=360-A;
			A = A / Math.PI * 180;
			double angel = x2 >= 0 ? A : 360 - A;
			// 三、计算与扇区的夹角abs1，abs= abs(azimuth -angle); 与扇区的夹角的修正，
			// 1）若：abs>180，abs1=360-abs，2）若：abs<=180,abs1=abs;其中azimuth为基站方向角
			double abs = Math.abs(a.getDirection_angle() - angel);
			double abs1 = abs > 180 ? 360 - abs : abs;
			// 四、距离修正：选择与扇区的夹角(abs1)小于60度的小区，
			// 然后根据基站类型，若是宏站，新距离=距离+3*abs1；若是室内站或街道站，距离不变。
			if (abs1 > 60)
				continue;
			if (a.getBts_type() != null && a.getBts_type().trim().equals("宏站"))
				a.setDistatce(a.getDistatce() + 3 * abs1);
			aList.add(a);
		}
		if (aList == null || aList.isEmpty())
			return null;
		// 五、根据计算新距离，选择最近五个小区。
		// aList = getStationsTopN(aList, 5);

		return getStationsTopN(aList, 5);
	}*/
	 ...
	 * @return
	 */
	/*public static List<AntennaInfo> getStationsTopN(List<AntennaInfo> list,
			int topN) {

		for (int j = list.size() - 1; j > 0; j--) {
			for (int i = 0; i < j; i++) {
				if (list.get(i).getDistatce() < list.get(i + 1).getDistatce()) { // 倒序
					AntennaInfo temp = list.get(i);
		...
		int start = 0;
		if (list.size() > start && list.size() > topN)
			start = list.size() - topN;
		// list = list.subList(start, list.size());

		return list.subList(start, list.size());
	}*/

	public static String normalizeAddress(String name, String address) {
		String[] specials = new String[3];
		specials[0] = "王医马弄";
		specials[1] = "管弄路";
		specials[2] = "花草弄";
		// 如果地址列为空，则取地名列
		if (ToolKit.isNullOrBlank(address))
			return name;
		...
		if (address.matches(".*[0-9]"))
			address = address.concat("弄");

		// 有些特殊地址中包含“弄”的，会影响匹配规则，需要特别处理
		...
		Pattern p = Pattern.compile("^[^号弄幢]*[0-9][号弄幢]");
		...
		if (ToolKit.isNullOrBlank(addr))
			addr = address;

		for (int i = 0; i < xzqDataList.length; i++) {
			// addr = addr.replaceFirst("^".concat(xzqData[i][0]), "");

			int idx = addr.indexOf(xzqDataList[i].trim());
			if (idx > -1)
				addr = addr.substring(idx + xzqDataList[i].length());
		}

		if (specName != null)
			addr = addr.replaceAll("&#", specName);

		// int idx = addr.indexOf("镇");
		// if (idx > -1) {
		// String zhen = addr.substring(0, idx) + "镇";
		//
		// boolean isExist = false;
		// for (int i = 0; i < xzqData.length; i++) {
		// if (zhen.equals(xzqData[i][0])) {
		// isExist = true;
		// break;
		// }
		// }
	...
	public static String parseOrderAddress(String address) {
		...
		if (address.indexOf("路") > -1) {
			// address=address.substring(address.indexOf("路")-5);
			for (String s : replaceList1) {
				if (address.indexOf(s) > -1) {
					address = address.substring(0, address.indexOf(s) + 1);
				}
			}
			for (String s : replaceList2) {
				if (address.indexOf(s) > -1) {
					address = address
							.substring(address.indexOf(s) + s.length());
				}
			}
			int lu_index = address.indexOf("路");
			for (String s : replaceList3) {
				int dis_index = address.indexOf(s);
				if (dis_index > -1) {
					if (lu_index - dis_index >= 4)
						address = address.substring(dis_index + 2);
				}
			}
			int zhen_index = address.indexOf("镇");
			if (zhen_index > -1) {
				if (lu_index - zhen_index >= 3)
					address = address.substring(zhen_index + 1);
			}

			if (address.length() >= 3 && address.indexOf("路") > -1)
				rslt = address;
		} else {
			for (String s : replaceList3) {
				int dis_index = address.indexOf(s);
	...
	public static void deleteAllSvnFiles(File dir) throws Exception {
	...
	public static void deleteAllFile(File f) throws Exception {
		...
		if (f.isDirectory()) {
			...
			for (int i = 0; i < ff.length; i++) {
				if (ff[i].isDirectory()) {
					String sss = ff[i].getAbsolutePath();
					all.add(sss);
				} else {
					allfile.add(ff[i].getAbsoluteFile());
				}
			}
			for (int n = 0; n < all.size(); n++) {
				String s1 = all.get(n).toString();
				f = new File(s1);
				ff = f.listFiles();
				for (int i = 0; i < ff.length; i++) {
					if (ff[i].isDirectory()) {
			...
			for (int i = 0; i < allfile.size(); i++) {
				String sa = allfile.get(i).toString();
				File fa = new File(sa);
				fa.delete();
			}
			for (int i = all.size() - 1; i >= 0; i--) {
				String sa = all.get(i).toString();
				File fa = new File(sa);
				fa.delete();
			}
		}
	...
	public static void main(String[] args) throws Exception {
		...
		// 递归显示C盘下所有文件夹及其中文件
		// File root = new File("C:\\Users\\huang\\Desktop\\北京gis\\新建文件夹
		// (2)\\arcgis_js_api");
		// deleteAllSvnFiles(root);

		// System.out.println(normalizeAddress("","大华商城"));
		// System.out.println(parseOrderAddress("大华商城"));梅川路1333
		// System.out.println(QtoB("彰武路１００号"))梅川路133317号;梅川路1333号;梅川路1333号11;梅川路1333号1233
		// String addr = "梅川路1333";
		// Pattern p = Pattern.compile("^[^号弄幢]*[0-9][号弄幢][^号弄幢]*[0-9][号弄幢]");
		// Matcher m = p.matcher(addr);
		// if (m.find()) {
		// System.out.println("yes");
		// int t = addr.indexOf("号");
		// if (addr.indexOf("号", t + 1) > -1)
		// addr = addr.substring(0, t) + "弄" + addr.substring(t + 1);
		// }
		// System.out.println(addr);
		// addr = "16323号8号";
		// if (addr.matches("[0-9\\s]{1,}[号弄幢]"))
		// System.out.println(addr);
		// else
		// System.out.println(1);

		// String[] rslt = new String[2];
		// DateFormat df_d = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		// try {
		//
		// Calendar c = Calendar.getInstance();
		// rslt[1] = df_d.format(c.getTime());
		// c.add(Calendar.MONTH, -3);
		// rslt[0] = df_d.format(c.getTime());
		// } catch (Exception e) {
		// e.printStackTrace();
		// }
